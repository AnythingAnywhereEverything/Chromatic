    ...
    React.useEffect(() => {
        if (externalPayload?.uploaded_avatar) {
            const blobUrl = URL.createObjectURL(
                externalPayload.uploaded_avatar,
            );
            setAvatarData((prev) => ({
                ...prev,
                blobUrl: blobUrl,
        ...
        if (externalPayload?.uploaded_banner) {
            const blobUrl = URL.createObjectURL(
                externalPayload.uploaded_banner,
            );
            setBannerData((prev) => ({
                ...prev,
                blobUrl: blobUrl,
        ...
        if (externalPayload?.display_name !== undefined || 
            externalPayload?.display_name === null
        ) {
            setDisplayName(externalPayload.display_name.length ? externalPayload.display_name : profile.username);
        } else {
            setDisplayName(
                profile.display_name ? profile.display_name : profile.username,
            );
    ...
    return (
        <div className={style["profile-body"]} ref={rootRef}>
            ...
            <div className={style["profile-info"]}>
                ...
                <div className={style["header-group"]}>
                    ...
                    <div className={style["profile-text"]}>
                        ...
                        />
                        <p className={style["profile-username"]}>
                            @{profile.username}
                        </p>
                    </div>
                </div>
            ...
            <div className={style["profile-interests"]}>
                Posts, Pinned, anything related to the user can be displayed
                here. This is a placeholder for future content.
            </div>
        </div>
    );
};
