...
impl TestContext {
    pub async fn new(pool: Option<PgPool>, test_storage_id: Option<&str>) -> Self {
        ...
        fs::create_dir_all(format!(
            "tests/media/{}",
            test_storage_id.unwrap_or("default")
        ))
        .unwrap();
        fs::create_dir_all(format!(
            "tests/temp/{}",
        ...
        test_config.media_temp_root =
            format!("tests/temp/{}", test_storage_id.unwrap_or("default"));
    ...
    #[allow(dead_code)]
    pub async fn generate_testimage(detect_ratio: (u32, u32)) -> (GeneratedTestImage, GeneratedTestVerification) {
