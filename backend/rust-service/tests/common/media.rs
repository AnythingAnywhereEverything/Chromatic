...
pub async fn generate_testimage(
    ...
    let mut canvas_opts = VOption::new();
    canvas_opts.add("bands", 3);

    let canvas = VipsImage::black_with_opts(width as i32, height as i32, canvas_opts)
        .expect("Failed to generate a test image base");

    let mut scale = [1.0, 1.0, 1.0];
    let mut red_offset = [255.0, 0.0, 0.0];

    let canvas = VipsImage::linear(&canvas, &mut scale[..], &mut red_offset[..])
    ...
    let max_left = width.saturating_sub(box_width);
    let max_top = height.saturating_sub(box_height);

    let box_left = if max_left == 0 {
        0
    } else {
        rand::random::<u32>() % (max_left + 1)
    };

    let box_top = if max_top == 0 {
        0
    } else {
        rand::random::<u32>() % (max_top + 1)
    };
    ...
    let green_box =
        VipsImage::black_with_opts(box_width as i32, box_height as i32, canvas_opts)
            .expect("Failed to generate a green box base");

    let mut green_offset = [0.0, 255.0, 0.0];
    let green_box = VipsImage::linear(&green_box, &mut scale[..], &mut green_offset[..])
        .expect("Failed to paint the green box");

    let final_image = VipsImage::insert(
        &canvas,
        &green_box,
        box_left as i32,
        box_top as i32,
    ...
    // * Position normalized to the available crop movement range.
    let normalized_position = (
        if max_left == 0 {
            0.0
        } else {
            box_left as f32 / max_left as f32
        },
        if max_top == 0 {
            0.0
        } else {
            box_top as f32 / max_top as f32
        },
    );
    ...
    let jpeg_bytes = VipsImage::jpegsave_buffer(&final_image)
    ...
    print!(
        ...
        (normalized_position.0 * max_left as f32).round() as u32,
        (normalized_position.1 * max_top as f32).round() as u32
    );
